...
def _get_v_mag_2d(hist: Dict[str, Any], species: str, i: int) -> np.ndarray:
    """Pomocná funkce: Vypočítá celkovou velikost rychlosti (|v|) ze složek vx a vy ve 2D."""
...
def _animate_velocity_distribution(hist: Dict[str, Any], plot_toggles: Dict[str, bool], animations: list):
    ...
    # Nalezení globálních extrémů velikosti rychlosti |v| pro fixaci osy
    v_mag_max = 0.0
    for species in ['e', 'i']:
    ...
    def animate_vel(i):
        ...
        max_y = 10
        if len(v_mag_e) > 1:
            ce, be = np.histogram(v_mag_e, bins=50, range=(v_mag_min, v_mag_max + pad))
            line_ve.set_data((be[:-1] + be[1:]) / 2, ce)
            max_y = max(max_y, ce.max())
        if len(v_mag_i) > 1:
            ci, bi = np.histogram(v_mag_i, bins=50, range=(v_mag_min, v_mag_max + pad))
