...
class DustImpactSimulation2D:
    def __init__(self, params: SimulationParams2D, toggles: SimulationToggles2D):
        ...
        # 1. Počáteční rozdělení částic (Emise ze sondy x=0)
        ...
        # Sestavení masek všech drátových antén na mřížce
        self.ant_masks = []
    ...
    def _build_poisson_solver(self):
        ...
        env_mode = "Plazma pozadí se stíněním" if self.toggles.enable_plasma_background else "Čisté vakuum bez stínění"
        print(f"Sestavuji Laplace/Poissonův řešič (Režim prostředí: {env_mode})")
    ...
    def _update_background_field(self, step: int = 0):
        ...
        for j in range(self.p.Ny):
            if np.abs(self.p.y_grid[j]) <= sonda_polomer:
                b_bg[self._get_1d_idx(0, j)] = self.p.Vf
            else:
        ...
        if not self.toggles.enable_background_field:
            self.V_bg_grid = np.zeros((self.p.Nx, self.p.Ny))
        else:
            V_bg_1d = self.solver_bg(b_bg)
            self.V_bg_grid = V_bg_1d.reshape((self.p.Nx, self.p.Ny))
    ...
    def _push_species(self, x: np.ndarray, y: np.ndarray, vx: np.ndarray, vy: np.ndarray,
        ...
        for a_idx, ant in enumerate(self.p.antennas):
            ...
            # Fyzická absorpce pro drátovou anténu
            ...
            # Indukovaný Ramo-Shockley proud
